...
export default function AdminProfile() {
  ...
  const uploadAvatar = () => {
    ...
    axios
      .post(`${API_BASE}/admin/profile/avatar`, fd, {
  ...
  const removeAvatar = () => {
    ...
    setMessage({ type: '', text: '' });
    axios
      .delete(`${API_BASE}/admin/profile/avatar`, { headers: headers() })
      .then(({ data }) => {
        setProfile(data.data || data);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`${API_BASE}/admin/profile`, payload, { headers: headers() })
      .then(({ data }) => {
        setProfile(data.data || data);
  ...
  const displayName = profile?.name || form.name || 'Admin';
      ...
      <div className="rounded-2xl bg-gradient-to-br from-amber-500/20 via-orange-500/10 to-stone-100 p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-white/80 px-4 py-3 shadow-sm">
            ...
            <span className="text-sm font-medium capitalize text-stone-600">{(profile?.role || 'admin').replace('_', ' ')}</span>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div>
                ...
                <dd className="font-medium capitalize text-stone-800">{(profile?.role || 'admin').replace('_', ' ')}</dd>
              </div>
              {profile?.created_at && (
                <div>
